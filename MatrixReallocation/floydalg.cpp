...
double* block_floyd_tiled(double* data, const int N, const int B)
{
    ...
    for (int t = 0; t < factor; ++t)
    {
        ...
        for (int k = 0; k < CBsize; ++k)
        {
            ...
            // указатель на к-ю строку центрального блока
            ...
            for (int b1 = 0; b1 < factor; ++b1)
            {
                ...
                for (int b2 = 0; b2 < factor; ++b2)
                {
                    ...
                    double* t_b2_block_k_line = data + t*B*N + b2*CBsize*B + k*ABwidth;
                    ...
                    {                     
                        double* Ai = ABi + i * ABwidth;
                        double Aik = b1_t_block[i*CBsize + k];
                        for (int j = 0; j < ABwidth; ++j)
                        {
